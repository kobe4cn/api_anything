...
use axum::Router;
use middleware::auth_guard::{self, AuthConfig};
use state::AppState;
...
pub fn build_app(state: AppState) -> Router {
    ...
    Router::new()
        ...
        // Axum layer 顺序从外到内：TraceLayer 最外层最先执行记录请求，
        // auth 在 trace 之后执行，保证即使认证失败的请求也有日志可查
        .layer(axum::middleware::from_fn(move |req, next| {
            let cfg = auth_config.clone();
            auth_guard::auth_middleware(cfg, req, next)
        }))
