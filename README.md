...
│                                                                                     │
│  ┌──────────────┐     ┌──────────────────┐     ┌────────────────────────────────┐   │
│  │  老系统接口    │     │  Generator Crate │     │        LLM (7 厂商可选)        │   │
│  │  定义文件      │     │  (代码生成引擎)    │     │  Claude/OpenAI/Gemini/GLM/    │   │
│  │              │     │                  │     │  Qwen/Kimi/DeepSeek           │   │
│  │  • WSDL      │────▶│  Stage 1: 读取    │────▶│                              │   │
│  │  • OData XML │     │  Stage 2: 发给LLM │◀───│  分析语义 + 生成 Rust 代码      │   │
│  │  • OpenAPI   │     │  Stage 3: 编译    │     │  • serde struct (强类型)       │   │
│  │  • CLI --help│     │  Stage 4: 测试    │     │  • 协议转换逻辑               │   │
│  │  • SSH 样例   │     │  Stage 5: 文档    │     │  • 错误处理                   │   │
│  │  • PTY 录制   │     │  Stage 7: 存储    │     │  • tracing 埋点               │   │
│  └──────────────┘     └────────┬─────────┘     └────────────────────────────────┘   │
│                                │                                                    │
│                    ┌───────────▼───────────┐                                        │
│                    │    产出物              │                                        │
│                    │  ┌─────────────────┐  │                                        │
│                    │  │ .so/.dylib 插件  │  │  ← LLM 生成的 Rust 代码编译而成        │
│                    │  │ (强类型,原生性能) │  │                                        │
│                    │  └────────┬────────┘  │                                        │
│                    │  ┌────────▼────────┐  │                                        │
│                    │  │ 路由 + 绑定信息  │  │  → 写入 PostgreSQL 元数据仓库           │
│                    │  └────────┬────────┘  │                                        │
│                    │  ┌────────▼────────┐  │                                        │
│                    │  │ OpenAPI 3.0 Spec│  │  → 自动生成 API 文档                    │
│                    │  └─────────────────┘  │                                        │
│                    └───────────────────────┘                                        │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
│                                                                                     │
│  ┌──────────────┐     ┌──────────────────────────────────────────────┐              │
│  │  外部调用方    │     │            Platform API (Axum)               │              │
│  │              │     │                                              │              │
│  │  • 移动App    │     │  ┌──────────────────────────────────────┐   │              │
│  │  • Web 前端   │────▶│  │         中间件管道                    │   │              │
│  │  • AI Agent  │ JSON│  │  TraceLayer → Auth → RateLimiter →   │   │              │
│  │  • 第三方系统  │ REST│  │  RequestLogger                       │   │              │
│  │              │     │  └──────────────┬───────────────────────┘   │              │
│  └──────────────┘     │                 │                           │              │
│                       │  ┌──────────────▼───────────────────────┐   │              │
│         ┌─────────────│  │      DynamicRouter (RCU 热加载)       │   │              │
│         │             │  │  /gw/banking/transfer → route_id_1   │   │              │
│         │    JSON     │  │  /gw/banking/balance  → route_id_2   │   │              │
│         │   响应返回   │  │  /gw/products         → route_id_3   │   │              │
│         │             │  └──────────────┬───────────────────────┘   │              │
│         │             │                 │ 匹配到 route_id           │              │
│         │             │  ┌──────────────▼───────────────────────┐   │              │
│  ┌──────▼──────┐      │  │     BackendDispatcher (保护层编排)    │   │              │
│  │  外部调用方  │      │  │                                      │   │              │
│  │  收到 JSON  │      │  │  ① 限流检查 (令牌桶)                  │   │              │
│  │  响应       │      │  │  ② 熔断检查 (Closed/Open/HalfOpen)   │   │              │
│  └─────────────┘      │  │  ③ 信号量获取 (并发控制)              │   │              │
│                       │  │  ④ 超时包装                           │   │              │
│                       │  └──────────────┬───────────────────────┘   │              │
│                       │                 │                           │              │
│                       │  ┌──────────────▼───────────────────────┐   │              │
│                       │  │  PluginManager 加载的 .so 插件        │   │              │
│                       │  │  (LLM 生成的强类型 Rust 代码)         │   │              │
│                       │  │                                      │   │              │
│                       │  │  transform_request:                  │   │              │
│                       │  │    JSON body → 老协议格式             │   │              │
...
│                       │  │                                      │   │              │
│                       │  │  execute:                            │   │──────┐       │
│                       │  │    reqwest POST → 老 SOAP 服务       │   │      │       │
│                       │  │    ssh command  → 老 SSH 服务器      │   │      │       │
│                       │  │    Command::new → 老 CLI 工具        │   │      │       │
│                       │  │    stdin/stdout → 老 PTY 程序        │   │      │       │
│                       │  │                                      │   │      │       │
│                       │  │  transform_response:                 │   │      │       │
│                       │  │    老协议响应 → JSON                  │   │      │       │
│                       │  │    <result>42</result>               │   │      │       │
...
外部请求 (JSON)                     老系统 (原始协议)
     │                                    ▲
     ▼                                    │
  /gw/banking/transfer              SOAP XML POST
     │                                    │
     ▼                                    │
  DynamicRouter                      reqwest::blocking
  (路由匹配 → route_id)                   │
     │                                    │
     ▼                                    │
  BackendDispatcher                  .so 插件
  (限流→熔断→信号量→超时)            (LLM 生成的 Rust 代码)
     │                                    │
     └──────────▶ Plugin.execute() ───────┘
                 JSON → SOAP XML → 发送 → SOAP XML → JSON
```

> **核心思路**：老系统完全不需要改动。API-Anything 在中间做"翻译"——LLM 理解老接口的协议和数据结构，生成一个专门的 Rust "翻译器"（.so 插件），这个翻译器把 JSON 请求转为老协议格式发给老系统，再把老系统的响应转回 JSON 返回给调用方。
